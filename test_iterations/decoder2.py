...
# Function to revert standard scaling of durations
def revert_standard_scaling(duration):
    mean_duration = -0.264648438
    std_duration = 0.803222656
    original_duration = duration * std_duration + mean_duration
    return int(round(original_duration))
...
for index, row in data.iterrows():
    ...
    for seq, duration in zip(sequences, durations):
        ...
        decoded_duration = revert_standard_scaling(duration)

        # Append the result to the list
        decoded_results.append([patient_id, body_part, decoded_sequences, decoded_duration])
...
decoded_df.to_csv("decoded_results.csv", index=False)
